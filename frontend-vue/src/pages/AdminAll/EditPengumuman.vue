<template>
    <div class="root-class">
        <div class="header">
            <h3 class="header-page">Edit Pengumuman</h3>
            ...
            <div class="modal fade" id="notification-success" tabindex="-1" role="dialog" aria-labelledby="sukses-setuju-modal" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    ...
                    <div class="modal-body">
                        ...
                        <p style="margin:0px 0px -15px 0px">Pengumuman berhasil diubah</p>
                        </div>
                    </div>
...
export default {
    ...
    mounted(){
        ...
        this.id_pengumuman = this.$route.params.id;

        UserService.getPengumumanById(this.id_pengumuman).then(
            response =>{
                this.judul = response.data.nama;
                this.deskripsi = response.data.deskripsi;
                this.file_pengumuman = response.data.file_pengumuman;
                if (this.file_pengumuman == null){
                    document.getElementById("file-saat-ini").style.display = "none";
                }
            },
            error => {
    ...
    methods: {
        submitPengumuman(){
            ...
            UserService.putPengumumanById(this.id_pengumuman, formData).then(
                response => {
